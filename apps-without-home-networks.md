...
This paper takes CMS's two stated rationales for home networks seriously, shows that each is better served by mechanisms that already exist or are already planned, addresses the app-developer-burden argument directly, and proposes concrete spec deltas.
...
Before arguing it away, inventory the load it carries. In the current draft, the home network is:
...
Each of these functions is real. None of them requires a designated home network to perform it. Several are performed *worse* by one.
...
The home-network model gives us the first. Every network that wants to serve as a home develops its own vetting process: its own test plan, its own evidence requirements, its own reviewers, its own renewal cycle. These processes will measure roughly the same things, none will be portable, and an app's "passing grade" will mean something slightly different at each one. We have run this experiment before in health IT, and the result is always the same: duplicated effort for reviewers, duplicated cost for the reviewed, and no increase in actual assurance.

The alternative already exists in working form:

- **Open reference test kits.** ONC's [Inferno](https://inferno.healthit.gov/) demonstrated the model: a public, executable definition of conformance that any party can run and any party can verify the results of. The CMS ecosystem should have the equivalent for its profiles (SMART App Launch, the RLS interaction, registration flows). A test suite that is open source and self-service is *strictly better* evidence of technical conformance than a network's bespoke review, because it is reproducible, versioned, and identical for everyone.
...
The conformance function the home network was supposed to serve is therefore better served without it: **one open bar, tested once, recognized everywhere**, instead of N private bars, tested N times, recognized nowhere else.
...
- **The asymmetry gets worse, not better.** The home network bears onboarding and support costs for its apps, while every *other* network is obligated to answer those apps' queries (§4.1, §4.3) and is forbidden from charging anyone for the patient-access traffic. Non-home networks do the serving; the home network holds the only revenue relationship. That is not cost recovery — it is a structural subsidy from everyone else to the home network.
...
- **It doesn't actually fund the queries.** The cost that matters at scale is data holders and networks serving patient-access traffic. A home-network fee paid by an app to one network does nothing for the other networks serving that app's queries. The funding problem is untouched; we've just added a tollgate that doesn't pay the road's bills.

Funding for patient-access infrastructure is a genuinely open problem, and this paper does not pretend to solve it. Two directions are worth naming so the working group doesn't reach for home networks as a false fix:
...
- **Use shared open-source libraries.** Most of the per-network variation is mechanical (see the [connectivity flows companion](app-connectivity-flows.md)). A well-maintained open-source client library absorbs it the way OAuth libraries absorbed provider quirks.
...
If the spec gets the floor right — Library listing confers connection authority; networks must accept the federal credential without duplicative gating — then **market competition delivers the convenience Liz is arguing for without the spec having to mandate any particular helper**. The home-network model delivers the same convenience only by making it compulsory, and compulsory convenience is just a gate with better marketing.
...
This case is argued for **patient-facing apps**, because the Medicare App Library exists today and CMS has committed to it. The current spec's home-network concept also covers payers, providers, and delegated tech solutions, where no CMS registry yet exists. Two honest options for those actor types:
...
Either way: do not let the unresolved general case drag the gate back in for the case that *is* resolved.
...
The [connectivity flows companion](app-connectivity-flows.md) shows an app traversing all three styles end-to-end — registration, record location, and data retrieval — with no home network anywhere in the path.
